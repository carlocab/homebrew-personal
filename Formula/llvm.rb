class Llvm < Formula
  ...
  def install
    ...
    install_args = std_cmake_args + args + %w[-DLLVM_TARGETS_TO_BUILD=all]

    bootstrap_args = args + %W[
      -DLLVM_TARGETS_TO_BUILD=Native
      -DCLANG_ENABLE_BOOTSTRAP=ON
      -DCLANG_BOOTSTRAP_CMAKE_ARGS='#{install_args.join(";")}'
    ]

    llvmpath = buildpath/"llvm"
    mkdir llvmpath/"build" do
      system "cmake", "-G", "Unix Makefiles", "..", *(std_cmake_args + bootstrap_args)
